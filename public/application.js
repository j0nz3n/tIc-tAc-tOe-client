...
var store = __webpack_require__(2);
var navBtns = __webpack_require__(13);
...
var gameLogic = __webpack_require__(34);
var gameAPI = __webpack_require__(6);
var ui = __webpack_require__(35);
var store = __webpack_require__(2);

var clickHandler = function clickHandler(event) {
  // Handles Game events // should be somewhere else
  gameLogic.clickEvent(event);
};

var new_game = function new_game(event) {
  event.preventDefault();
  // console.log('New game ran!')

  gameAPI.newGame(event).then(ui.newGameSuccess).catch(ui.newGameFailure);
};

var all_games = function all_games(event) {
  event.preventDefault();
  // console.log('All Games ran!')
  gameAPI.getAllGames(event).then(ui.allGamesSuccess).catch(ui.newGameFailure);
};
...
var playerMove = function playerMove(target, token) {
  turn ? xMoves.push(target) : oMoves.push(target);
  var thingclicked = document.getElementById(target);
  var index = thingclicked.dataset.index;
  var value = token;
...
var checkForWin = function checkForWin(playerMoves) {
  var winScenario = [["1", "2", "3"], // top row
  ["4", "5", "6"], // middle row
  ["7", "8", "9"], // bottom row
  ["1", "4", "7"], // left column
  ["2", "5", "8"], // middle column
  ...
  var scenarioChecks = winScenario.map(function (scenario) {
    // Grab each subArray in winScenario
    var winningMoves = playerMoves.filter(function (move) {
      // Grab playerMoves and checks if
      return scenario.indexOf(move) !== -1; // that is a winning
    });
    return winningMoves.length === 3;
  });

  var didWin = scenarioChecks.some(function (scenarioCheck) {
    return scenarioCheck === true;
  });
  if (didWin === true) {
    gameOver = true;
    turn ? ($('#game-message').text('X WINS!'), setTimeout(function () {
      $('#game-message').text("");
    }, 1000)) : ($('#game-message').text('O WINS!'), setTimeout(function () {
      $('#game-message').text("");
    }, 2000));
...
module.exports = __webpack_require__.p + "3d1d6097db8ef893e726d0059c12885d.png";
